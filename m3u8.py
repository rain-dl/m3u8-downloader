...
class Downloader:
    ...
    def _download(self, ts_list):
        ...
        dowork =  partial(self._worker, uniqueid)
    ...
    def _worker(self, uniqueid, ts_tuple):
        url = ts_tuple[0]
        index = ts_tuple[1]
        retry = self.retry
        ...
        while retry:
            try:
                ...
                if r.ok:
                    ...
                    self.ts_finish += 1
                    print (original_file_name+'\t|\t'+r.headers['content-length']+'B'+'\t|\t'+str(self.ts_finish)+'/'+str(self.ts_total))
...
if __name__ == '__main__':
    if (len(sys.argv)==1):
        ...
        if len(sys.argv) == 2:
            print ('m3u8.py <m3u8_url> <download_path> -o <out_file> -s <start_time> -e <end_time> -f <start_file> -g <end_file> [-u]')
            sys.exit(2)
        else:
    ...
    try:
        ...
        print ('高级配置参数: -o <out_file> -s <start_time> -e <end_time> -f <start_file> -g <end_file> [-u]')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print ('m3u8.py <m3u8_url> <download_path> -o <out_file> -s <start_time> -e <end_time> -f <start_file> -g <end_file> [-u]')
    ...
    if outfile != '': print ("[Out File]:", outfile)
